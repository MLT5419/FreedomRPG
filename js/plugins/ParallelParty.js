 ...
 *
 * @param ShareResource
 * @desc 队伍之间共享资源（道具、武器、护甲、步数、金币）(ON/OFF)
 * @default OFF
 *
 * @param SavePosition
 * @desc 切换队伍时保留原始位置，返回时自动切换位置。
 * @default OFF
 *
 * @help 你可以同时管理多个队伍。
 * 队伍由队伍ID管理，初始状态下队伍ID为0。
 * 道具和金币也分开管理，通过插件指令进行切换。
 * 
 *
 * 切换到新队伍会清空角色，所以请手动用事件指令添加。
 * 
 *
 * 角色信息是共享的，如果加入的角色来自另一个队伍，则继承状态。
 * 
 *
 * 战斗中的队伍无法被替换。
 *
 ...
 *  （参数之间用半角空格分隔）
 *
 * PP_パーティ変更 1 OFF # 更换队伍为1
 * PP_CHANGE_PARTY 1 OFF # 同上
 * PP_パーティ変更 1 ON  # 更换队伍为1并合并资源
 * PP_CHANGE_PARTY 1 ON  # 同上
 *
 * 资源整合为ON，则物品和金币会合并进当前队伍。
 * 多个队伍合并资源时可以使用。
 * 如果资源共享参数有效，则不会发生合并。
 *
