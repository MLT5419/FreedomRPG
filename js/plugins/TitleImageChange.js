 ...
 *
 * @param 優先度変数
 * @desc 当有多个保存数据时，确定哪个保存数据进度优先的变量编号（1 ...）。 
 * @default 0
 * @type variable
 *
 * @param タイトル1の進行度
 * @desc 如果进度变量的值大于或等于该值，则显示标题 1 的图像。 
 * @default 1
 *
 * @param タイトル1の画像
 * @desc 进度变量的值大于或等于标题1的进度时显示的图像的文件名（img / titles1）。 
 * @default
 * @require 1
 * @dir img/titles1/
 * @type file
 *
 * @param タイトル1のBGM
 * @desc 进度变量的值等于或高于标题1的进度时播放的BGM（audio/bgm）的文件名。
 * @default
 * @require 1
 * @dir audio/bgm/
 * @type file
 *
 * @param タイトル2の進行度
 * @desc 如果进度变量的值大于或等于该值，则显示标题 2 的图像。 
 * @default 2
 *
 * @param タイトル2の画像
 * @desc 进度变量的值大于或等于标题2的进度时显示的图像的文件名（img / titles1）。 
 * @default
 * @require 1
 * @dir img/titles1/
 * @type file
 *
 * @param タイトル2のBGM
 * @desc 进度变量的值等于或高于标题2的进度时播放的BGM（audio/bgm）的文件名。 
 * @default
 * @require 1
 * @dir audio/bgm/
 * @type file
 *
 * @param タイトル3の進行度
 * @desc 如果进度变量的值大于或等于该值，则显示标题 3 的图像。 
 * @default 3
 *
 * @param タイトル3の画像
 * @desc 进度变量的值等于或高于标题3的进度时显示的图像的文件名（img / titles1）。 
 * @default
 * @require 1
 * @dir img/titles1/
 * @type file
 *
 * @param タイトル3のBGM
 * @desc 进度变量的值等于或高于标题3的进度时播放的BGM（audio/bgm）的文件名。 
 * @default
 * @require 1
 * @dir audio/bgm/
 * @type file
 *
 * @param 以降の進行度
 * @desc 如果要使用 4 种以上的标题模式，请用逗号分隔它们来指定进度。 示例 (4,5,6) 
 * @default
 *
 * @param 以降の画像
 * @desc 如果要使用 4 个或更多标题，请指定以逗号分隔的图像文件名 (img / titles1)。 示例 (aaa, bbb, ccc) 
 * @default
 *
 * @param 以降のBGM
 * @desc 如果要使用 4 个或更多标题模式，请指定以逗号分隔的 BGM（音频/背景音乐）文件名。 示例 (aaa, bbb, ccc) 
 * @default
 *
 * @help 根据游戏进度更改标题画面的图像和BGM。
 * 进度可以指定任何变量，通常反映所有保存数据中的最大值。 
 *
 * 但是，如果单独指定优先级变量，
 * 则根据变量值最大的保存数据的进度来确定图像和BGM。 
 *
 * 最多可以指定 3 个标题图像，如果满足多个条件，
 * 则优先顺序如下。 
 *
 * 1. 第4张图及以上的图片和BGM  //注：变量最好使用代入而不是增加
 * 2. 标题 3 图像和 BGM 
 * 3. 标题 2 图像和 BGM 
 * 4. 标题 1 图像和 BGM 
 * 5. 默认标题图片和 BGM
